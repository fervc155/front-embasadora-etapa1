<template>
  <div class="va-page-not-found justify--center pb-5" :style="pageNotFoundStyle" v-bind="$attrs">
    <div class="va-page-not-found__inner align--center">
      <slot name="image"/>
      <div class="va-page-not-found__title text--center mb-4">{{('404.title')}}</div>
      ...
      <slot/>
      <!-- <va-button v-if="!withoutButton" :to="{ name: 'dashboard' }">{{('404.back_button')}}</va-button> -->
    </div>
    <made-by-component/>
...
export default {
  ...
  computed: {
    ...
    pageNotFoundStyle () {
      return {
        // color: this.contextConfig.invertedColor ? this.hemes.dark : 'white',
        color: 'var(--va-gray)',
        // backgroundColor: this.contextConfig.invertedColor ? 'white' : this.hemes.danger,
    ...
    wallpaperColor () {
      // return this.contextConfig.invertedColor ? this.hemes.dark : '#e4ff32'
